...
-- Note: to get only the single top product line per gender, wrap this in a
-- window function (ROW_NUMBER() OVER (PARTITION BY gender ORDER BY purchase_count DESC))
-- if your SQL dialect supports it.
